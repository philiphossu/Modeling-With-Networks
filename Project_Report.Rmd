...
- id: Textbook
  title: A First Course in Mathematical Modeling 5th Edition
  author:
  - family: Giordano
    given: Frank
  - family: Fox
  ...
  - family: Horton
    given: Steven
  container-title: Cengage Learning
  type: article
  issued:
    year: 2013
...
\item Define and study a critical measurement by which to determine the influence of authors in the network we created in part a. 
\item Gather data, build, and analyze a network showing the relationship between some foundational papers in the emerging field of network science. Apply the influence measures used for parts a,b and discuss their effectiveness. Also, discuss methodology and other factors surrounding this network. 
\item Gather data, build, and analyze a real life scenario which can be modeled as a network. Again apply influence measures and discuss the results, shortcomings, external factors, etc.
\item Discuss how influence and impact can be used in real life situations. Consider business decisions, improving influence, selecting a graduate school etc. 
\end{itemize}
...
Another way to visualize this graph and consider the degree of each vertex is via an adjacency matrix. This is a V x V matrix where the value of each entry denotes the presence (1) or absence (0) of an edge between the two corresponding vertices. The value in each entry can be modified to contain the edge weight. The corresponding adjacency matrix for our running example is shown below. 
...
We utilize the igraph package in R which provides an easy to use interface through which to perform this calculation [@igraph]. The following code block demonstrates the calculation on our example using igraph.
...
## 4. Analysis and Testing of the Models
...
Here, we create the Erdos1 co-author network after cleaning and importing the data. We make this network undirected since our data only shows some form of collaboration between authors. Prior to testing our metrics on this network, we explore some of the interesting properties of this network as suggested in the project requirements. 
...
According to the eigenvector centrality, \textit{TSFCN} is also the most influential paper in the citation network. A plot for the network is provided in Figure 2, where the vertices are colored based on their eigenvector centrality. A higher eigenvalue is mapped to a warmer color.
...
After importing and processing the data for this graph (omitted from our report), we begin by creating the actors collaboration network. The data we used is available at https://www.kaggle.com/sudheersankar/movie-metadata/data. This is a sizable data set which contains entries about various modern movies. We clean this data to focus on just the actors who are collaborating in a certain film. The data set contains other interesting information such as critic ratings, Facebook likes, etc. which could be utilized in further analysis. 
...
The alpha centrality model accounts for all the problems, because it can handle directed edges. Unlike the eigenvector centrality model, in order to determine the influence of a paper, alpha centrality considers the influence of the papers which cites it. Because the citation network we used was built around 1 paper (\textit{On Random Graphs}), we expected this paper to have the highest influence in the network, and the results obtained with the alpha centrality model matched our expectations. 
...
\begin{enumerate}
\item If an actor has collaborated with few actors but for many times, that actor should not gain more influence but this model counts duplicate collaborations towards influence.
\item The definition is not accurate enough. In other words, it does not reflect the real world definition of influence.
\end{enumerate}
...
###5.4 Applying Influence Metrics to Real World Decisions
One final discussion to consider is that of taking our influence metrics and applying them to real life decision scenarios. It’s apparent that networks can model several real-world scenarios, from trade between countries to investment in a network of banks to even co-author networks such as the Erdos1 co-author network we presented and analyzed. Additionally, a measurement of influence in a network could certainly make an impact on decision making in the real world. Consider some the takeaways from the Total Degree Model vs the Eigenvector Centrality model in our Erdos1 network. If an individual wanted to increase their academic influence in their field, they would focus on collaborating with other influential authors, rather than simply writing as many papers as they could. Similarly, if an up-and-coming actor wanted to increase their influence in Hollywood, they would want to have their agent set up a film with a big-name actor rather than multiple films with smaller actors. Evidently, neither of these goals are trivial in practice, but these examples show the need of effective models that can measure influence in a network.
...
\item After some time has passed (perhaps a year) create an updated version of the network, adding the new papers which cite already present papers in the network
\item Analyze changes in network and re-run metrics
\end{enumerate}
